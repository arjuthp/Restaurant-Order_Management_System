...
export const staffApi = {
  ...
  getAllStaff: async (filters?: StaffFilters): Promise<StaffResponse> => {
    const response: any = await apiClient.get('/staff', { params: filters });
  ...
  getStaffById: async (id: string): Promise<Staff> => {
    const response: any = await apiClient.get(`/staff/${id}`);
    return response.data || response;
  },

  // Create staff
  createStaff: async (data: CreateStaffData): Promise<Staff> => {
    const response: any = await apiClient.post('/staff', data);
    return response.data || response;
  },

  // Update staff
  updateStaff: async (id: string, data: UpdateStaffData): Promise<Staff> => {
    const response: any = await apiClient.put(`/staff/${id}`, data);
    return response.data || response;
  },

  // Toggle staff status
  toggleStaffStatus: async (id: string, isActive: boolean): Promise<Staff> => {
    const response: any = await apiClient.patch(`/staff/${id}/status`, { isActive });
    return response.data || response;
  },
};
